...
public:
    ...
    */
    ...
    void mset( VecTupleString &value );

    uint8_t msetnx( VecTupleString &value );
	 ...
	 */
    bool blpop( const VecString &key , uint64_t &timeout , VecTupleString &value );
	 ...
	 * @return 没元素弹出返回false，否则返回true
	 */
    bool brpop( const VecString &key , uint64_t &timeout , VecTupleString &value );
    ...
    uint64_t hgetall( const string& key , VecTupleString& pairs );
    ...
    void hmset( const string& key , const VecTupleString& pairs );
	 ...
	 */
    bool hscan( const string& key , int64_t &cursor , VecTupleString& values , const string& match =
			"" , uint64_t count = 0 );
     ...
     */
    uint64_t  zadd(const string& key,const VecTupleString& map);
    /**
    ...
    uint64_t zrangeWithscore(const string& key,const int64_t start,const int64_t stop,VecTupleString& reply);
    /**
    ...
    uint64_t zrangebyscoreWithscore(const string& key,const string& min,const string& max,VecTupleString& reply,int64_t offset=0,int64_t count=0);
    /**
    ...
    uint64_t zrevrangeWithscore(const string& key,const int64_t start,const int64_t stop,VecTupleString& reply);
    /**
    ...
    uint64_t zrevrangebyscoreWithscore(const string& key,const string& max,const string& min,VecTupleString& reply,int64_t offset=0,int64_t count=0);
    /**
     ...
     */
    bool zscan( const string& key, int64_t &cursor, VecTupleString& reply, const string& match="", uint64_t count=0 );
    /**
    ...
    uint64_t psubnumsub( VecTupleString& value,const VecString& channel = VecString() );
...
protected:
    ...
    void _getStringTupleFromArry( const CResult::ListCResult& arry , VecTupleString& pairs );
    ...
    bool _getArry(Command &cmd , VecTupleString& pairs, uint64_t &num  );
